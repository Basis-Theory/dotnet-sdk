...
public partial class OwnerClient : IOwnerClient
{
    ...
    private async Task<WithRawResponse<TenantMemberResponse>> TransferAsyncCore(
    ...
    /// </code></example>
    public WithRawResponseTask<TenantMemberResponse> TransferAsync(
        TransferTenantOwnerRequest request,
        IdempotentRequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<TenantMemberResponse>(
            TransferAsyncCore(request, options, cancellationToken)
        );
    }
}
