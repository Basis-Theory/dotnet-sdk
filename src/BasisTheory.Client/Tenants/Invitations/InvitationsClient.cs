...
public partial class InvitationsClient
{
    ...
    /// </code>
    /// </example>
    public Pager<TenantInvitationResponse> ListAsync(
        ...
        var response = await _client
            .MakeRequestAsync(
                new RawClient.JsonApiRequest
                {
                    ...
                    Method = HttpMethod.Post,
                    Path =
                        $"tenants/self/invitations/{JsonUtils.SerializeAsString(invitationId)}/resend",
                    Options = options,
                },
                cancellationToken
            )
        ...
        var response = await _client
            .MakeRequestAsync(
                new RawClient.JsonApiRequest
                {
                    ...
                    Path = $"tenants/self/invitations/{JsonUtils.SerializeAsString(invitationId)}",
                    Options = options,
                },
                cancellationToken
            )
    ...
    private async Task<TenantInvitationResponsePaginatedList> ListAsync(
        InvitationsListRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            try
            {
                return JsonUtils.Deserialize<TenantInvitationResponsePaginatedList>(responseBody)!;
            }
            catch (JsonException e)
            {
                throw new BasisTheoryException("Failed to deserialize response", e);
