...
public partial class PaymentMethodsClient : IPaymentMethodsClient
{
    ...
    {
        _client = client;
        Rails = new global::BasisTheory.Client.Agentic.PaymentMethods.RailsClient(_client);
    }
    ...
    /// </summary>
    private WithRawResponseTask<PaymentMethodList> ListInternalAsync(
        PaymentMethodsListRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<PaymentMethodList>(
            ListInternalAsyncCore(request, options, cancellationToken)
        );
    }

    private async Task<WithRawResponse<PaymentMethodList>> ListInternalAsyncCore(
        PaymentMethodsListRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        var _queryString = new global::BasisTheory.Client.Core.QueryStringBuilder.Builder(
            ...
            .Add("consumer_id", request.ConsumerId)
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                var responseData = JsonUtils.Deserialize<PaymentMethodList>(responseBody)!;
                return new WithRawResponse<PaymentMethodList>()
                {
                    Data = responseData,
    ...
    private async Task<WithRawResponse<PaymentMethod>> CreateAsyncCore(
    ...
    private async Task<WithRawResponse<PaymentMethod>> GetAsyncCore(
    ...
    )
    {
        return new WithRawResponseTask<SharedPaymentProviderErrorList>(
            ErrorsInternalAsyncCore(paymentMethodId, request, options, cancellationToken)
        );
    }
        ...
        var response = await _client
            .SendRequestAsync(
                new JsonRequest
                {
                    Method = HttpMethod.Get,
                    Path = string.Format(
                        "agentic/payment-methods/{0}/errors",
                        ValueConvert.ToPathParameterString(paymentMethodId)
                    ),
                    QueryString = _queryString,
                    Headers = _headers,
    ...
    /// );
    /// </code></example>
    public WithRawResponseTask<PaymentMethod> CreateAsync(
        CreatePaymentMethodRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    ...
    /// </code></example>
    public WithRawResponseTask<PaymentMethod> GetAsync(
        string paymentMethodId,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<PaymentMethod>(
            GetAsyncCore(paymentMethodId, options, cancellationToken)
        );
    }
    ...
    )
    {
        return new WithRawResponseTask(
            DeleteAsyncCore(paymentMethodId, options, cancellationToken)
        );
    }
    ...
    )
    {
        ...
        >
            .CreateInstanceAsync(
                ...
                options,
                async (request, options, cancellationToken) =>
                    await ErrorsInternalAsync(paymentMethodId, request, options, cancellationToken)
                        .WithRawResponse(),
                (request, cursor) =>
                {
                    request.Start = cursor;
