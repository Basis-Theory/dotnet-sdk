...
public partial class EnrichmentsClient : IEnrichmentsClient
{
    ...
    private async Task<WithRawResponse<CardDetailsResponse>> CardDetailsAsyncCore(
    ...
    /// </code></example>
    public WithRawResponseTask<CardDetailsResponse> CardDetailsAsync(
        EnrichmentsCardDetailsRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<CardDetailsResponse>(
            CardDetailsAsyncCore(request, options, cancellationToken)
        );
    }
}
