...
public partial class ReactorformulasClient
{
    private RawClient _client;

    internal ReactorformulasClient(RawClient client)
    {
        _client = client;
    }
    ...
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            try
            {
                return JsonUtils.Deserialize<ReactorFormula>(responseBody)!;
            }
            catch (JsonException e)
            {
                throw new BasisTheoryException("Failed to deserialize response", e);
    ...
    /// </code>
    /// </example>
    public async Task<ReactorFormula> GetAsync(
        string id,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
